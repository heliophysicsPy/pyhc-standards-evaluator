...
- **Duplication**: ✅ SWVO is not a fork and reuses upstream clients where they exist (e.g., the MIDL reader wraps the official `csem-midl` package). OMNI, Kp, Dst, and F10.7 download/read functionality overlaps with SpacePy (`spacepy.omni`), PySPEDAS, and HAPI clients, but the GFZ‑specific products (Kp/Hp ensemble forecasts, SWIFT and ENLIL ensemble solar wind, PAGER plasmasphere, Niemegk nowcasts, MIDL merged L1 record) and the unified `BaseIO` reader interface are not available elsewhere.
